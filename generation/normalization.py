...
1. Use the "==" symbol to show the relationship, with the time period reference on the left and the corresponding date on the right.
2. If no temporal date reference is found, the output should be 'N/A', without any other information.
3. If multiple time period references are found, separate them using '|'.
4. The output should not contain both exact dates and "N/A".
5. Do not include outputs like {temporal expression} == "N/A" or "| {temporal expression} == N/A".
6. The output should use either "YYYY-MM-DD", "YYYY-MM", or "YYYY" to represent the exact date.
...
unlimited_dataset_generator = OpenAIDatasetGenerator(
    temperature=0.3, requests_per_minute=90, responses_per_request=3
)
